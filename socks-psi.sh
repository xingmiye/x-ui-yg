...
ins() {
    # 清理旧进程
    [ -f /usr/local/psiphon/wpphid.log ] && kill -15 $(cat /usr/local/psiphon/wpphid.log) >/dev/null 2>&1
    
    # 架构检测和下载
    if [ ! -e /usr/local/psiphon/wpph ]; then
        case $(uname -m) in
            aarch64) cpu=arm64 ;;
            x86_64)  cpu=amd64 ;;
        ...
        if ! curl -L -o /usr/local/psiphon/wpph --retry 2 --insecure "https://raw.githubusercontent.com/yonggekkk/x-ui-yg/main/xuiwpph_$cpu"; then
            red "文件下载失败!" && exit 1
        fi
        chmod +x /usr/local/psiphon/wpph
    fi
...
unins() {
    [ -f /usr/local/psiphon/wpphid.log ] && kill -15 $(cat /usr/local/psiphon/wpphid.log) >/dev/null 2>&1
    rm -f /usr/local/psiphon/wpph.log /usr/local/psiphon/wpphid.log
    
    # 清理定时任务
    crontab -l | grep -v wpphid.log | crontab -
...
while true; do
    ...
    case $menu in
        1)
            ins
            nohup setsid /usr/local/psiphon/wpph -b 127.0.0.1:$port --gool -$sw46 >/dev/null 2>&1 &
            echo "$!" > /usr/local/psiphon/wpphid.log
            ...
            if [[ -n $resv1 || -n $resv2 ]]; then
                echo "/usr/local/psiphon/wpph -b 127.0.0.1:$port --gool -$sw46 >/dev/null 2>&1" > /usr/local/psiphon/wpph.log
                # 设置开机启动
                crontab -l | grep -v wpphid.log | {
                    cat
                    echo "@reboot /bin/bash -c 'nohup setsid $(cat /usr/local/psiphon/wpph.log) & echo \$! > /usr/local/psiphon/wpphid.log'"
                } | crontab -
...
CO-哥伦比亚      PE-秘鲁
            ...
            nohup setsid /usr/local/psiphon/wpph -b 127.0.0.1:$port --cfon --country $guojia -$sw46 >/dev/null 2>&1 &
            echo "$!" > /usr/local/psiphon/wpphid.log
            ...
            resv1=$(curl -sx socks5://localhost:$port icanhazip.com)
            if [[ -n $resv1 ]]; then
                echo "/usr/local/psiphon/wpph -b 127.0.0.1:$port --cfon --country $guojia -$sw46 >/dev/null 2>&1" > /usr/local/psiphon/wpph.log
                # 设置开机启动
                crontab -l | grep -v wpphid.log | {
                    cat
                    echo "@reboot /bin/bash -c 'nohup setsid $(cat /usr/local/psiphon/wpph.log) & echo \$! > /usr/local/psiphon/wpphid.log'"
